...
@app.get("/api/videos")
async def get_videos(type: Optional[str] = None, category_id: Optional[str] = None, limit: int = 50):
    query = {}
...
async def get_related_videos(video_id: str, user_id: str = Depends(get_user_id)):
    ...
    # Simple related logic: same category or tags
    query = {
        "_id": {"$ne": parse_id(video_id)},
        "type": {"$ne": "short"},
        "$or": [
